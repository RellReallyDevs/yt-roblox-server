...
app.post("/discord/player-joined", requireSecret, async (req, res) => {
  ...
  if (!joinChannelId) {
    log("WARN  JOIN_CHANNEL env var not set — skipping player-joined post");
...
// NEWS (APITube)
// ─────────────────────────────────────────────────────────────────────────────
const APITUBE_KEY  = process.env.APITUBE_KEY || "api_live_1rgt5ipe40o6gyaoshA7OeALFvZ2dLi9EJ01NXxYp6BbfRpsoycjAgtkbMz";
const NEWS_BASE    = "https://api.apitube.io/v1/news/everything";
const NEWS_TOPICS  = [
...
let newsCache = { articles: [], lastUpdated: null };

const refreshNews = async () => {
  try {
    const results = await Promise.all(
      NEWS_TOPICS.map(async (topic) => {
        const url = `${NEWS_BASE}?${topic.params}&per_page=5&sort.by=published_at&sort.order=desc&api_key=${APITUBE_KEY}`;
        const res  = await fetch(url);
        if (!res.ok) {
          const body = await res.text();
          throw new Error(`APITube ${res.status} for ${topic.label}: ${body.slice(0, 200)}`);
        }
    ...
    newsCache = { articles: results.flat(), lastUpdated: new Date().toISOString() };
    log(`NEWS  Cached ${newsCache.articles.length} articles`);
  } catch (e) {
...
refreshNews();
setInterval(refreshNews, 7 * 60 * 1000); // refresh every 7 minutes

app.get("/news", (_req, res) => {
  res.json({ lastUpdated: newsCache.lastUpdated, count: newsCache.articles.length, articles: newsCache.articles });
});
